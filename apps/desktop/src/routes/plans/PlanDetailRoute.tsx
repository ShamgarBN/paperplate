    ...
    mutationFn: async (params: {
      ...
      (async () => {
        ...
        const { data: row } = await supabase
          .from("recipes")
          .select("base_servings, preferred_servings")
...
async function buildCuisineByRecipeMap(): Promise<Map<number, string | null>> {
  const { data, error } = await supabase
    ...
    .select("recipe_id, category:categories!inner(kind, name)")
    .eq("category.kind", "cuisine");
  if (error) throw error;
  const map = new Map<number, string | null>();
  for (const row of (data ?? []) as any[]) {
    const cat = Array.isArray(row.category) ? row.category[0] : row.category;
    if (cat && !map.has(row.recipe_id)) map.set(row.recipe_id, cat.name);
  }
  return map;
}
